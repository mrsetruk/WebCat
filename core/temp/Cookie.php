...
class Cookie
{
    ...
    public static function isCookieValid(){
        ...
        if (self::$hashedCookie === hash('sha256', self::$userId . ':' . self::$token . ConfigurationOld::get('COOKIE_SECRET_KEY')) && !empty(self::$token) && !empty(self::$userId)) {
    ...
    public static function remove($userId = null){
        ...
        setcookie('auth', false, time() - (3600 * 3650), ConfigurationOld::get('COOKIE_PATH'), ConfigurationOld::get('COOKIE_DOMAIN'), ConfigurationOld::get('COOKIE_SECURE'), ConfigurationOld::get('COOKIE_HTTP'));
    }
    /**
     * Reset Cookie,
    ...
    public static function reset($userId)
    {
        ...
        self::$hashedCookie = hash('sha256', self::$userId . ':' . self::$token . ConfigurationOld::get('COOKIE_SECRET_KEY'));
        $authCookie = $cookieFirstPart . ':' . self::$hashedCookie;
        setcookie('auth', $authCookie, time() + ConfigurationOld::get('COOKIE_EXPIRY'), ConfigurationOld::get('COOKIE_PATH'), ConfigurationOld::get('COOKIE_DOMAIN'), ConfigurationOld::get('COOKIE_SECURE'), ConfigurationOld::get('COOKIE_HTTP'));
    }
}