...
class Encryption
{
    ...
    private static function getCharacters(){
        ...
        $key_hash = hash('sha256', ConfigurationOld::get('HASH_KEY'));
        $key_hash = (strlen($key_hash) < strlen($chars) ? hash('sha512', ConfigurationOld::get('HASH_KEY')) : $key_hash);
        for ($n = 0; $n < strlen($chars); $n++) {
    ...
    public static function encrypt($plain){
        ...
        $key = mb_substr(hash(self::HASH_FUNCTION, ConfigurationOld::get('ENCRYPTION_KEY') . ConfigurationOld::get('HMAC_SALT')), 0, 32, '8bit');
